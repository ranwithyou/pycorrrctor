...
def read_vocab(input_texts, max_size=None, min_count=0):
    ...
    for texts in input_texts:
        for token in texts:
            token_counts.update(token)  # 统计每个词的数目
    # 按值排序字数
    count_pairs = token_counts.most_common()  # most_common(n):用于显示前n个最多的元素
...
def gen_examples(src_sentences, batch_size):
    ...
    print("句子长度为：%s" % len(src_sentences))
    examples = []
...
#             vec = ' '.join(list(map(str, vec)))#map会根据提供的函数对指定序列做映射
#             f.write('{} {}\n'.format(word, vec))


def write_config(filename, **kwargs):
    with open(filename, mode='w', encoding='utf-8') as f:
        json.dump(kwargs, f)  # json.dump()是把python对象转换成json对象生成一个fp的文件流，和文件相关。


def read_config(filename):
    with open(filename, mode='r', encoding='utf-8') as f:
        return json.load(f)  # 将文件中大的json格式转换为python对象提取出来
